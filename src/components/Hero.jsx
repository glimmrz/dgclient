...
const Hero = () => {
  ...
  useEffect(() => {
    const element = document.querySelector(
      '[data-w-id="0019d883-cf8f-a8a3-c6d3-ee9730fe4092"]'
    );
    if (element) {
      element.style.willChange = "transform, opacity"; // Optimizing for performance
      element.style.transition =
        "transform 0.5s ease-in-out, opacity 0.5s ease-in-out"; // Adding transition
      setTimeout(() => {
        element.style.transform = "none";
        element.style.opacity = "1";
      }, 100); // Adding a small delay to ensure it applies correctly
    }
  }, []);
